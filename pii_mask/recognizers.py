...
def find_format_entities(text: str, org_names: tuple[str, ...] = ()) -> list[Entity]:
    ...
    # Названия из словаря разносятся тем же механизмом, что и найденные регуляркой:
    # источник детерминированный (человек назвал их явно), поэтому размножение по
    # документу безопасно - в отличие от находок NER, где одно ложное срабатывание
    # размножилось бы по всему тексту.
